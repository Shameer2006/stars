...
export default function StarOfDay({ namedStars }: StarOfDayProps) {
  ...
  return (
    <motion.div
      ...
      className="w-full max-w-lg mx-auto px-3 sm:px-4 mb-2 sm:mb-4 z-20 relative"
    >
      <Link href={`/star-of-the-day/${encodeURIComponent(star.proper || '')}`}>
        <div
          className="flex flex-row items-center gap-3 sm:gap-4 p-3 sm:p-4 rounded-2xl border transition-all duration-300 hover:scale-[1.02] hover:shadow-xl cursor-pointer"
          style={{
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-0.5">
              <span className="text-[8px] sm:text-[10px] uppercase tracking-[0.2em] text-amber-300 font-sans font-semibold">
                ★ Star of the Day
              </span>
              <span className="text-[7px] sm:text-[9px] text-gray-300 uppercase tracking-widest bg-white/10 px-1.5 py-0.5 rounded-full">
                Learn More ↗
              </span>
            </div>
            
            <div className="text-base sm:text-lg md:text-xl font-cinzel text-amber-400 truncate tracking-wide mt-0.5">
              {star.proper}
            </div>
            
            <div className="flex flex-wrap gap-x-2 sm:gap-x-3 gap-y-0.5 mt-0.5 sm:mt-1 text-[10px] sm:text-xs text-gray-300 font-cormorant tracking-wide">
              <span>{distLy}</span>
            ...
            <p className="text-[10px] sm:text-xs text-gray-300 mt-1 sm:mt-1.5 line-clamp-2 leading-relaxed font-sans">
              {fullDescription}
            </p>
          </div>
        </div>
